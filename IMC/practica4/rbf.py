...
def entrenar_rbf(fichero_train, fichero_test, num_rbf, clasificacion, eta, l2):
    ...
    if not clasificacion:
        ...
        print cnf_matrix
        index = np.where(prediccion_test != test_outputs)
        print index
...
def lectura_datos(fichero_train, fichero_test):
    ...
    test_outputs = file2.values[:, -1:]"""
        ...
        # Cargar el dataset
    train_inputs, train_outputs = load_svmlight_file(fichero_train)
    test_inputs, test_outputs = load_svmlight_file(fichero_test)
...
def inicializar_centroides_clas(train_inputs, train_outputs, num_rbf):
    ...
    clases = []
    num_clases = len(np.unique(train_outputs[:]))
...
if __name__ == "__main__":
    ...
    for s in range(10,60,10):
        ...
        train_mses[s/10-1], test_mses[s/10-1], train_ccrs[s/10-1], test_ccrs[s/10-1] = \
            entrenar_rbf(fichero_train='./basesDatos/libsvm/train_spam.libsvm', 
                         fichero_test='./basesDatos/libsvm/test_spam.libsvm', num_rbf=100, clasificacion=True, eta=0.1, l2=True)
