...
void PerceptronMulticapa::propagarEntradas(bool softmax) {
	...
	for (int i = 1; i < nNumCapas; ++i)//La capa 0 es la entrada
	{
		for (int j = 0; j < pCapas[i].nNumNeuronas; ++j)
		{
			...
			//cout<<"Exponete "<<exponente<<endl;
			if(softmax && i==(nNumCapas-1)){
		...
		//cout<<"Esponente "<< pCapas[i].pNeuronas[0].x<<endl;
	}
	...
	if(softmax){
		...
		//cout<<"Exponte final "<< pCapas[nNumCapas-1].pNeuronas[0].x<<endl;
...
void PerceptronMulticapa::retropropagarError(double* objetivo, int funcionError, bool softmax) {
	//Solo capa de salida
	if (softmax)
	{
...
void PerceptronMulticapa::simularRedOnline(double* entrada, double* objetivo, int funcionError, bool softmax) {
	for (int i = 1; i < nNumCapas; ++i)
	{
		for (int j = 0; j < pCapas[i].nNumNeuronas ; ++j)
		{
	...
	propagarEntradas(softmax);
	retropropagarError(objetivo, funcionError, softmax);
	acumularCambio();
	ajustarPesos();
...
// Simular la red: propagar las entradas hacia delante, computar el error, retropropagar el error y ajustar los pesos
// entrada es el vector de entradas del patrón y objetivo es el vector de salidas deseadas del patrón
void PerceptronMulticapa::simularRedOffline(double* entrada, double* objetivo, int funcionError, bool softmax) {
...
void PerceptronMulticapa::entrenarOnline(Datos* pDatosTrain, int funcionError, bool softmax) {
	int i;
	for(i=0; i<pDatosTrain->nNumPatrones; i++)
		simularRedOnline(pDatosTrain->entradas[i], pDatosTrain->salidas[i], funcionError, softmax);
}
...
void PerceptronMulticapa::entrenarOffline(Datos* pDatosTrain, int funcionError, bool softmax) {
	for (int i = 1; i < nNumCapas; ++i)
	{
		for (int j = 0; j < pCapas[i].nNumNeuronas ; ++j)
		{
	...
	for(int i=0; i<pDatosTrain->nNumPatrones; i++)
		simularRedOffline(pDatosTrain->entradas[i], pDatosTrain->salidas[i], funcionError, softmax);
...
void PerceptronMulticapa::entrenar(Datos* pDatosTrain, int funcionError, bool online, bool softmax) {
	if(online)
		entrenarOnline(pDatosTrain, funcionError, softmax);
	else
		entrenarOffline(pDatosTrain, funcionError, softmax);
}

// ------------------------------
// Probar la red con un conjunto de datos y devolver el error MSE cometido
double PerceptronMulticapa::test(Datos* pDatosTest, int funcionError, bool softmax) {
	int i;
...
double PerceptronMulticapa::testClassification(Datos* pDatosTest, bool softmax){
	...
	for(int i=0; i<pDatosTest->nNumPatrones; i++){
		...
		int maxIndex= 0, maxIndexD=0;
		alimentarEntradas(pDatosTest->entradas[i]);
		propagarEntradas(softmax);
		for (int j = 0; j < pCapas[nNumCapas-1].nNumNeuronas; ++j)
		{
			//cout<<"DValor x "<< pDatosTest->salidas[i][j]<< " y max "<<maxValueD<<endl;
			if (pDatosTest->salidas[i][j] > maxValueD)
			{
				maxValueD = pDatosTest->salidas[i][j];
				maxIndexD = j;
			}
			//cout<<"Valor x "<< pCapas[nNumCapas-1].pNeuronas[j].x<< " y max "<<maxValue<<endl;
			if (pCapas[nNumCapas-1].pNeuronas[j].x > maxValue)
			{
				//cout<<"ENTRA"<<endl;
				maxValue = pCapas[nNumCapas-1].pNeuronas[j].x;
				maxIndex = j;
			}
			//cout<<"Neurona "<<j<< " valor "<< pCapas[nNumCapas-1].pNeuronas[j].x<<endl;
		}
		//cout<<"Vuelta "<<i<< " indice "<< maxIndex<< " y "<<maxIndexD<<endl;
		if (maxIndex == maxIndexD)
		{
			dAvgTestError += 1.0;
		}
...
void PerceptronMulticapa::ejecutarAlgoritmo(Datos * pDatosTrain, Datos * pDatosTest, int maxiter, double *errorTrain, double *errorTest, double *ccrTrain, double *ccrTest, int funcionError, bool online, bool softmax)
{
	int countTrain = 0;
	...
	do {
		entrenar(pDatosTrain, funcionError, online, softmax);
		double trainError = test(pDatosTrain, funcionError, softmax);
		testError = test(pDatosTest, funcionError, softmax);
	...
	*ccrTrain = testClassification(pDatosTrain, softmax);
	*ccrTest = testClassification(pDatosTest, softmax);
